...
void main()
{
    ...
    float SSAO1 = texture2DRect(u_ssaoTex, gl_FragCoord.xy).r;
    //gl_FragColor = vec4(SSAO1);
    ...
    for(int i = 0; i < 8; i++)
    {
        vec2 depthTexCoord = vec2(texcoord.x + Offsets[i] * u_inv_screenSize.x, texcoord.y);
